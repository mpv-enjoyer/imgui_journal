...
Popup_Select_Day_Of_The_Week::Popup_Select_Day_Of_The_Week(JournalHolder* graphical)
: Popup(graphical)
{
...
bool Popup_Select_Day_Of_The_Week::show_frame()
{
    if (begin_frame("Выбрать день недели"))
    {
        for (int i = 1; i < 7; i++)
        {
            ...
            if (Graphical::button_selectable(button_name.c_str(), day_of_the_week == i)) day_of_the_week = i;
            ImGui::SameLine();
        }
        //now for russian-styled calendar
        std::string button_name = generate_label(Day_Names[0] + "##change_day_button", {});
        if (Graphical::button_selectable(button_name.c_str(), day_of_the_week == 0)) day_of_the_week = 0;
