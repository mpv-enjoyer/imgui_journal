...
public:
    ...
    {
        const Month current_month = Month::make_current();
        if (model->students()->cref_students().cref(m_student_pos).is_removed(current_month))
        {
            return "Невозможно добавить удаленного ученика";
        }
        auto& merged_lesson = model->cref_merged_lesson(m_merged_lesson_id);
        if (merged_lesson.is_removed(current_month)) return "Невозможно добавить ученика к удаленному уроку";
