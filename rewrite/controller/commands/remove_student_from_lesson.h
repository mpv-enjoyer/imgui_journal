...
public:
    ...
    {
        if (model->cref_merged_lesson(m_merged_lesson_id).is_student_removed(m_student_pos, Month::make_current())) return "Ученик уже удален";
        return {};
    }
    void call(IModel& model) override
    {
        model->ref_merged_lesson(m_merged_lesson_id).remove_student(m_student_pos, Month::make_current());
    }
};
...
public:
    ...
    std::optional<std::string> get_error(const IModel& model) const override
    {
        if (!model->cref_merged_lesson(m_merged_lesson_id).is_student_removed(m_student_pos, Month::make_current())) return "Ученик ещё не удален";
        return {};
    }
    void call(IModel& model) override
    {
        model->ref_merged_lesson(m_merged_lesson_id).restore_student(m_student_pos, Month::make_current());
    }
};