...
class Move_Student : public ICommand
{
    ...
    Position<Attendance_Student> m_from_student_pos;
    Merged_Lesson_ID m_to_merged_lesson_id;
    Position<Student> get_student_pos(const IModel& model) const
    {
        return model->attendance_wdays()->cref_merged_lesson(m_from_merged_lesson_id).get_student_positions()[m_from_student_pos.get()];
    }
public:
